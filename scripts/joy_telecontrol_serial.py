...
def generate_random_pose_delta(args, dt: float, current_phase_time: float) -> tuple[np.ndarray, float]:
    """조이스틱 대신 부드러운 랜덤 움직임(Delta)을 생성합니다."""
    delta = np.zeros(7, dtype=np.float32)

    # 예시: 0.5초마다 새로운 무작위 방향 설정 (로봇이 덜덜 떠는 것 방지)
    # 실제 구현 시에는 클래스 멤버나 전역 변수로 방향을 유지하는 것이 좋습니다.
    if current_phase_time > 0.5:
        # X, Y, Z축에 대해 무작위 속도 바운드 지정
        random_v = np.random.uniform(-0.4, 0.4, size=3)
        delta[0] = random_v[0] * args.pos_gain * dt  # X
        delta[1] = random_v[1] * args.pos_gain * dt  # Y
        delta[2] = random_v[2] * args.pos_gain * dt  # Z
        current_phase_time = 0.0

    # 그리퍼도 가끔 열고 닫도록 무작위 샘플링
    if np.random.rand() < 0.05:
        delta[6] = np.random.choice([-args.gripper_rate, args.gripper_rate]) * dt

    return delta, current_phase_time
...
def teleop_loop(args, robot: XArm6ServoCartesian, cameras: dict[str, RealSenseCamera] | None, dataset) -> None:
    ...
    try:
        # teleop_loop 내부의 주요 변경 구간 예시
        phase_time = 0.0
        while True:
            t0 = time.time()

            # [중요] 조이스틱 입력 대신 랜덤 함수 호출!
            delta7, phase_time = generate_random_pose_delta(args, control_dt, phase_time)
            phase_time += control_dt
            # ... 기존의 클리핑(clamp_target_pose) 및 로봇 전송(send_target_pose)은 그대로 유지하여 안전 확보!
            ...
            if dataset is not None and now >= next_record_t:
                ...
                # d3il 스타일에 맞게 mm 단위를 m 단위로 변환 가공 예시
                d3il_state = state7.copy()
                d3il_state[0:3] /= 1000.0  # mm -> m

                d3il_action = target_pose7.copy()
                if args.action_mode == "delta":
                    d3il_action = record_delta_action_from_target(state7, target_pose7)
                    d3il_action[0:3] /= 1000.0  # 델타 거리도 m 단위로 변환
